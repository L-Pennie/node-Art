...
export default class baseComponent {
    constructor() {
            this.idList = ['user_id', 'cate_id', 'question_id', 'answer_id', 'anno_id', 'token_id', 'slide_id', 'work_id', 'goods_id']
        }
    ...
    async getImgUrl(folder, base) {
            let img_arr = []
            if (base != null) {
                for (let i = 0; i < base.length; i++) {
                    if (base[i].indexOf('data') === 0) {
                        let name = Date.now() + '.png'
                            // 设置图片的生成路径
                            // let ps = '../wd-app-server/public/images/' + name
                        let ps = path.join(__dirname, `../public/images/${folder}/${name}`)
                            //去掉图片base64码前面部分data:image/png;base64
                        base[i] = base[i].replace(/^data:image\/\w+;base64,/, "");
                        //把base64码转成buffer对象，
                        let dataBuffer = new Buffer(base[i], 'base64');
                        // 用fs写入文件
                        await this.write(ps, dataBuffer)
                        img_arr.push(`http://${config.public_host}:${config.port}/public/images/${folder}/${name}`)
                    } else {
                        img_arr.push(base[i])
                    }
                }
